...
FString UReadWriteFile::ReadStringFromFile(FString FilePath, bool& bOutSuccess, FString& OutInfoMessage)
{
    ...
    {
        bOutSuccess = false;
        OutInfoMessage = FString::Printf(TEXT("Read String From File Failed: Unable to read file. Is it a text file? - '%s'"), *FilePath);
        return "";
    }
...
void UReadWriteFile::WriteStringToFile(FString FilePath, FString String, bool& bOutSuccess, FString& OutInfoMessage)
{
    ...
    {
        bOutSuccess = false;
        OutInfoMessage = FString::Printf(TEXT("Write String To File Failed: Unable to write to file. Is it read only? If the path valid? - '%s'"), *FilePath);
        return;
    }
